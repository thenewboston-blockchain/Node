...
class MessageMixin:
    ...
    def make_binary_data_for_cryptography(self) -> bytes:
        return self.json(separators=(',', ':'), sort_keys=True).encode('utf-8')  # type: ignore

    def make_hash(self) -> Hash:
        normalized_message = self.make_binary_data_for_cryptography()
    ...
    def make_binary_data_and_signature(self, signing_key: SigningKey):
        binary_data = self.make_binary_data_for_cryptography()
