...
# TODO(dmu) HIGH: Mongo is not a model. Model is a business object to operate on
#                 https://thenewboston.atlassian.net/browse/BC-136
class Mongo:
    ...
    def _update_accounts(self, accounts):
        for account_number, account_data in accounts.items():
            self.accounts_collection.update_one({'_id': account_number}, {'$set': account_data}, upsert=True)

    def _update_config_from_block_data(self, *, block_identifier, block_number):
        config = {'last_block_hash': block_identifier, 'last_block_number': block_number}
    ...
    def _update_nodes(self, nodes):
        for account_number, node_data in nodes.items():
            self.nodes_collection.update_one({'_id': account_number}, {'$set': node_data}, upsert=True)

    def _update_schedule(self, validators):
        for position, validator in validators.items():
            self.schedule_collection.update_one({'_id': position}, {'$set': validator}, upsert=True)
    ...
    def update_config(self, *, config: dict):
        self.config_collection.update_one({'_id': '0'}, {'$set': config}, upsert=True)
